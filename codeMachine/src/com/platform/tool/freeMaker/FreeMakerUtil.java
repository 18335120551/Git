...
public class FreeMakerUtil {
	...
	public Template getTemplate(String name) {
		try {
			...
			/*
			 * 当处理某个模版时,FreeMarker直接从缓存中返回对应的Template
			 * 对象,并有一个默认的机制来保证该模版对象是跟模版文件同步的template_update_delay
			 * 用来指定更新模版文件的间隔时间,相当于多长时间检测一下是否有必要重新加载模版文件,0
			 * 表示每次都重新加载,否则为多少毫秒钟检测一下模版是否更改. cfg.setTemplateUpdateDelay(0);
			 */
	...
	public void generateFile(String templateName, Map<String, Object> root, String outFilePath) {
		FileWriter out = null;
		try {
			// 通过文件输出流,将文件写入到相应的目录中,此处用的是绝对路径
